...
/// <item>this[i]: i-th D3 collection.</item>
/// <item>this[i,j,k,l]: element at the (i,j,k,l)-th position.</item>
/// <item>GetOrNone(i,j,k,l): returns Some of the element at the (i,j,k,l)-th position if indices are valid; None otherwise.</item>
/// <item>AsEnumerable(): returns the collection as IEnumerable; particularly useful for linq calls over the collection.</item>
/// </list>
/// 
/// </summary>
public class UniJaggedD4<T>
{
    // data
    internal readonly Func<int, int, int, int, T> Get;
    /// <summary>
    ...
    ///     }
    /// };
    /// UniJaggedD4&lt;int> jagged = new(array);
    /// Assert(jagged.Length1() == 1);
    /// </code>
    /// </summary>
    ...
    /// };
    /// UniJaggedD4&lt;int> jagged = new(array);
    /// Assert(jagged.Length4(0, 0, 1) == 3);
    /// </code>
    /// </summary>
    public readonly Func<int, int, int, int> Length4;
    /// <summary>
    /// Safely gets Some of the element at the (i,j,k,l)-th position; returns None if the indices are invalid.
    /// 
    /// <code>
    /// var array = new int[1][][][]
    /// {
    ...
    /// UniJaggedD4&lt;int> jagged = new(array);
    /// 
    /// Assert(jagged.GetOrNone(0, 1, 0, 2) == Some(2));
    /// Assert(jagged.GetOrNone(1, 0, 0, 0).IsNone);
    /// Assert(jagged.GetOrNone(0, 1, 0, 4).IsNone);
    /// </code>
    /// 
    /// For other methods on the resulting optional, see <see cref="Opt{T}"/>.
    /// </summary>
    public readonly Func<int, int, int, int, Opt<T>> GetOrNone;
    /// <summary>
    ...
    /// </summary>
    public readonly Func<IEnumerable<IEnumerable<IEnumerable<IEnumerable<T>>>>> AsEnumerable;
    // data - constant
    /// <summary>
    ...
    // ctor
    internal UniJaggedD4(int length1, Func<int, int> length2, Func<int, int, int> length3, Func<int, int, int, int> length4, Func<int, int, int, int, T> get, Func<int, int, int, int, Opt<T>> getOrNone, Func<IEnumerable<IEnumerable<IEnumerable<IEnumerable<T>>>>> asEnumerable, Opt<T> underlyingConstantValue)
    {
        Length1 = () => length1;
        Length2 = length2;
        Length3 = length3;
    ...
    public UniJaggedD4(T constantValue, Opt<int> length1 = default, Opt<Func<int, int>> getLength2 = default,
        Opt<Func<int, int, int>> getLength3 = default, Opt<Func<int, int, int, int>> getLength4 = default)
    {
        ...
        Get = (_, _, _, _) => constantValue;
        GetOrNone = (_, _, _, _) => Some(constantValue);
    ...
    /// <param name="getLength3">Optional function (i,j -> length) to get length of the (i,j)-th collection; when omitted will default to None which will yield to a length of int.MaxValue for any indices.</param>
    /// <param name="getLength4">Optional function (i,j,k -> length) to get length of the (i,j,k)-th collection; when omitted will default to None which will yield to a length of int.MaxValue for any indices.</param>
    public UniJaggedD4(Func<int, int, int, int, T> getValue, Opt<int> length1 = default, Opt<Func<int, int>> getLength2 = default,
        Opt<Func<int, int, int>> getLength3 = default, Opt<Func<int, int, int, int>> getLength4 = default)
    {
        var len1 = () => length1.UnwrapOr(int.MaxValue);
        var len2 = getLength2.UnwrapOr(i => int.MaxValue);
    ...
    public UniJaggedD4(List<List<List<List<T>>>> list)
    {
        int len1() => list.Count;
    ...
    /// </summary>
    /// <param name="list">Underlying list of the unified jagged collection.</param>
    public UniJaggedD4(IList<IList<IList<IList<T>>>> list)
    {
        int len1() => list.Count;
        int len2(int i) => list[i].Count;
        int len3(int i, int j) => list[i][j].Count;
    ...
    /// <summary>
    /// 4-dimensional jagged collection lengths and values of which are determined by the underlying <paramref name="enumerable"/>.
    /// 
    /// <para>
    ...
    public UniJaggedD4(IEnumerable<IEnumerable<IEnumerable<IEnumerable<T>>>> enumerable)
    {
        ...
        Get = (i, j, k, l) => enumerable.ElementAt(i).ElementAt(j).ElementAt(k).ElementAt(l);
        GetOrNone = (i, j, k, l) =>
        {
            if (i > -1 && i < len1())
            {
                if (j > -1 && j < len2(i))
                {
                    ...
                    {
                        if (l > -1 && l < len4(i, j, k))
                            return Some(enumerable.ElementAt(i).ElementAt(j).ElementAt(k).ElementAt(l));
                    }
                }
            }
            return None<T>();
    ...
    // index1
    /// <summary>
    /// Returns the <paramref name="i"/>-th D3 collection.
    /// <code>
    /// var array = new int[1][][][]
    /// {
    ///     new int[2][][]
    ...
    /// 
    /// int i = 0;
    /// UniJaggedD3&lt;char> jaggedI = jagged[i];
    /// Assert(jaggedI.Length1() == jagged.Length2(i) == 2);
    /// Assert(jaggedI[1, 2] == jagged[0, 1, 2] == 2);
    /// </code>
    /// </summary>
    /// <param name="i">Index of the D3 collection to create.</param>
    public UniJaggedD3<T> this[int i]
    {
        get
        {
            var get = Get;
            ...
            var length3 = Length3;
            var length4 = Length4;
            return new(Length2(i), j => length3(i, j), (j, k) => length4(i, j, k),
                (j, k, l) => get(i, j, k, l),
                (j, k, l) => getOrNone(i, j, k, l),
                () => asEnumerable().ElementAt(i),
                UnderlyingScalar);
        }
    }
    /// <summary>
    /// Directly returns the element at the (i,j,k,l)-th position.
    ...
    /// UniJaggedD4&lt;int> jagged = new(array);
    /// 
    /// Assert(jagged[0, 1, 0, 2] == 2);
    /// // var number = jagged[1, 0, 0, 0]; // out-of-bounds, throws!
    /// // var number = jagged[0, 1, 0, 4]; // out-of-bounds, throws!
    /// </code>
    /// </summary>
